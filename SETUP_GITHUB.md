...
process: every ~10 minutes a runner spins up, restores the SQLite database
from the Actions cache, runs 2 poll cycles (120 s apart), saves the database
back, and exits. A second workflow produces the daily report and a database
backup. Public repos get unlimited Actions minutes — that is what makes this
free, and it is why the repo must be **public**.
...
| Database | grows unbounded on disk | pruned to 7 days (`SNAPSHOT_RETENTION_DAYS`), persisted via Actions cache, gzipped backup artifact daily (30-day retention) |
...
~18 h before start, per the 2026-06-12 backtest): cron jitter, the coarser
cadence, and a small risk of cache eviction (mitigated by the daily backup
artifact — download, `gunzip`, and it becomes the seed for a new cache).
The fresh empty database also means the first day may re-alert matches the
local bot already alerted on — one-time noise.
